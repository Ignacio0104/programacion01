...
float askForCuit ()
{
	...
	if(pedirFloatIntentosRango(&requestedId, 0, 100000, 3, "Ingrese el ID: ", "Error, dato ingresado invalido")==0)
	{
		retorno=requestedId;
	} else
	{
...
int cont_remove (eContratacion*contList, int lenght, float cuitIngresado)
{
	int retorno;
	char userChoice;
	...
	if(contList!=NULL&&lenght>0)
	{
		...
		posicionSolicitada=cont_buscarPorCuit (contList, lenght, cuitIngresado);

		if(posicionSolicitada>0)
		{
			printf("\nSe va a eliminar la siguiente contratacion: \n\n"
					"ID: %d. CUIT: %f. Cantidad de dias: %d.   Nombre: %s. \n\n",
					contList[posicionSolicitada].idPantalla,contList[posicionSolicitada].cuitCliente,
